...
-- CUSTOM CODE
hasRadio = function(cb)
	cb(true)
end

hasRadio = function(cb)
	if (ESX == nil) then return cb(0) end
	ESX.TriggerServerCallback('esx_inventoryhud:getItemAmount', function(quantity)
		cb(quantity > 1)
	end, 'radio')
end
...
AddEventHandler("Radio.Toggle", function()
	...
	hasRadio(function(hasRadio)
		if hasRadio == true then
			if not isFalling and not isDead and Radio.Enabled then
				Radio:Toggle(not Radio.Open)
			end
		else
			ESX.ShowNotification('Нямаш радиостанция!')
		end
	end)
end)
--
